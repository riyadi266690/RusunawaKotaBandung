...
@section('content')
    <style>
        .card-lokasi-container>.col-md-4:nth-child(n+4) {
            ...
            /* Nilai 1.5rem setara dengan mt-4 di Bootstrap */
        }
    </style>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <p class="text-muted text-center mb-4 pb-2">Temukan lokasi Rusunawa yang sesuai dengan minat dan kebutuhan anda.
            </p>
            <div class="container">
                        ...
                        <div class="col-md-4 stretch-card grid-margin grid-margin-md-0">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <h5 class="text-success text-center mb-4">Tersedia {{ $lok->unit_available_count }} Unit
                                    </h5>
                                    <table class="mx-auto">
                                        <tr>
                                            <td><i data-feather="check" class="icon-md text-primary me-2"></i></td>
                                            <td>
                                                <p>Area Terbuka Hijau</p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i data-feather="check" class="icon-md text-primary me-2"></i></td>
                                        ...
                                        <tr>
                                            <td><i data-feather="check" class="icon-md text-primary me-2"></i></td>
                                            <td>
                                                <p>Bebas Biaya Pendaftaran</p>
                                            </td>
                                        </tr>
                                    </table>
                                    <div class="d-grid">
                                        <button type="button" class="btn btn-success mt-4 btn-daftar-lokasi"
                                            data-bs-toggle="modal" data-bs-target="#registrationModal"
                                            data-lokasi-id="{{ $lok->id }}" data-lokasi-nama="{{ $lok->nama_lokasi }}"
                                            data-link-formulir="{{ $lok->link_formulir }}"{{-- BARIS BARU --}}
                                            data-status-formulir="{{ $lok->status_formulir }}">
                                            Daftar Sekarang
                                        </button>
                                    </div>
                                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="registrationModalLabel">Form Pendaftaran Lokasi: <span
                            id="lokasiNamaDisplay"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="forms-sample form-adi" action="{{ route('pendaftaran.store') }}" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="telp_pendaftar" name="telp_pendaftar"
                                placeholder="No Telp / WhatsApp" required>
                        </div>
                        <div class="mb-3">
                            <label for="suket" class="form-label" id="labelSuket">Unggah Formulir Pendaftaran (Hanya
                                PDF)</label>
                            <input type="file" class="form-control" id="suket" name="suket"
                                placeholder="Pilih File" accept=".pdf">
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (registrationModal) {
                registrationModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    var statusFormulir = button.getAttribute('data-status-formulir');
                    ...
                    var linkDownload = registrationModal.querySelector(
                        '#linkFormulirDownload'); // BARIS BARU

                    var inputSuket = document.getElementById('suket');
                    var labelSuket = document.getElementById('labelSuket');
                    ...
                    if (statusFormulir === 'wajib') {
                        inputSuket.setAttribute('required', 'required');
                        labelSuket.innerHTML =
                            'Unggah Formulir Pendaftaran <span class="text-danger">(Wajib)</span>';
                    } else {
                        inputSuket.removeAttribute('required');
                        labelSuket.innerHTML =
                            'Unggah Formulir Pendaftaran <span class="text-muted">(Tidak Wajib)</span>';
                    }
                });
            }
        });
