...
@section('content')
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-baseline mb-2">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                            data-bs-target="#addLokasiModal" id="addLokasiBtn">
                            Tambah Lokasi Baru
                        </button>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-baseline mb-2">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                            data-bs-target="#addGedungModal" id="addGedungBtn">
                            Tambah Gedung Baru
                        </button>
                    </div>
                    <div class="table-responsive">
            ...
            <div class="card overflow-hidden">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-baseline mb-4 mb-md-3">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                            data-bs-target="#addUnitModal" id="addUnitBtn">
                            Tambah Unit Baru
                        </button>
                    </div>
                    <div class="table-responsive">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addLokasiForm" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="modal fade" id="addGedungModal" tabindex="-1" aria-labelledby="addGedungModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <script>
            ...
            $('#DTGedung').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        data: 'lokasi',
                        name: 'lokasi'
                    }, // Ini akan menampilkan nama lokasi
                    {
                        data: 'aksi',
                        name: 'aksi',
                        orderable: false,
            ...
            $('#DTUnit').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        data: 'gedung',
                        name: 'gedung'
                    }, // Ini akan menampilkan nama gedung
                    {
                        data: 'nomor',
                        name: 'nomor'
                    },
        ...
        `);
            ...
            window.editLokasi = function(id) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        console.error('AJAX Error fetching lokasi data for edit:', xhr
                            .responseText);
                    }
                });
            };
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    $('#DTGedung').DataTable().ajax.reload(null,
                                        false); // Reload Gedung juga
                                    $('#DTUnit').DataTable().ajax.reload(null,
                                        false); // Reload Unit juga
                                } else {
                            ...
                            error: function(xhr) {
                                Swal.fire('Gagal!',
                                    'Terjadi kesalahan saat menghapus data lokasi.',
                                    'error');
                                console.error('AJAX Error deleting lokasi data:', xhr
                                    .responseText);
                            }
                        });
                    }
            ...
            $('#addLokasiForm').on('submit', function(event) {
                ...
                const lokasiId = $('#lokasiId').val();
                const formMethod = $('#lokasiFormMethod').val(); // POST / PUT

                let url = storeLokasiUrl;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            let errorMessage = response.message || 'Gagal menyimpan data.';
                            if (response.errors) {
                                errorMessage += '<br>' + Object.values(response.errors)
                                    .map(e => `- ${e[0]}`).join('<br>');
                            }
                            Swal.fire('Gagal!', errorMessage, 'error');
                        }
            ...
            window.editGedung = function(id) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        console.error('AJAX Error fetching gedung data for edit:', xhr
                            .responseText);
                    }
                });
            };
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                Swal.fire('Gagal!',
                                    'Terjadi kesalahan saat menghapus data gedung.',
                                    'error');
                                console.error('AJAX Error deleting gedung data:', xhr
                                    .responseText);
                            }
                        });
                    }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            Swal.fire('Gagal!', 'Terjadi kesalahan saat menghapus data unit.',
                                'error');
        ...
        $('#addUnitForm').on('submit', function(event) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    if (xhr.responseJSON && xhr.responseJSON.errors) {
                        errorMessage += '<br>' + Object.values(xhr.responseJSON.errors).map(e =>
                            `- ${e[0]}`).join('<br>');
                    }
                    Swal.fire('Gagal!', errorMessage, 'error');
